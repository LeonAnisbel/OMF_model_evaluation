...
def pd_combine_pol(OMF_pd, plot_dir):
    ...
    obs_omf_pol = (list(OMF_pd['OMF_mod_poly'].values) +
                   list(OMF_pd['OMF_obs_poly_sub'].values))

    obs_omf_all = (list(OMF_pd['OMF_mod_tot'].values) +
                   list(OMF_pd['OMF_obs_tot_sub'].values))

    ID_all = (list(OMF_pd['ID'].values) +
              list(OMF_pd['ID'].values))
    ...
    names = ['NAO', 'CVAO', 'WAP']
    dict_comp = dict((name, {
        'pol_mod': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_poly_sub'])['OMF_mod_poly'].values),
        'pol_obs': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_poly_sub'])['OMF_obs_poly_sub'].values),
        'tot_mod': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_tot_sub'])['OMF_mod_tot'].values),
        'tot_obs': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_tot_sub'])['OMF_obs_tot_sub'].values)
        }) for name in names)
    # print(dict_comp)
...
def pd_combine_pr_li(pd_obs_mo_cvao, plot_dir):
    ...
    obs_omf_pro_lip = list(pd_obs_mo_cvao['OMF_mod_pro'].values) + list(pd_obs_mo_cvao['OMF_mod_lip'].values) + \
                      list(pd_obs_mo_cvao['OMF_obs_prot_sub'].values) + list(pd_obs_mo_cvao['OMF_obs_lipi_sub'].values)

    ID_all = (list(pd_obs_mo_cvao['Name_pro'].values) + list(pd_obs_mo_cvao['Name_lip'].values) +
              list(pd_obs_mo_cvao['Name_pro'].values) + list(pd_obs_mo_cvao['Name_lip'].values))  # Prot or lip
...
def pd_combine_pro_lip(OMF_pd, plot_dir):
    ...
    obs_omf_pro = (list(OMF_pd_pro['OMF_mod_pro'].values) +
                   list(OMF_pd_pro['OMF_obs_prot_sub'].values))

    obs_omf_lip = (list(OMF_pd_lip['OMF_mod_lip'].values) +
                   list(OMF_pd_lip['OMF_obs_lipi_sub'].values))

    ID_pro = (list(OMF_pd_pro['ID'].values) +
              list(OMF_pd_pro['ID'].values))

    ID_lip = (list(OMF_pd_lip['ID'].values) +
              list(OMF_pd_lip['ID'].values))
    ...
    dict_comp_pro = dict((name, {'pro_mod': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_prot_sub'])[
                                                     'OMF_mod_pro'].values),
                                 'pro_obs': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_prot_sub'])[
                                                     'OMF_obs_prot_sub'].values), }) for name in names)
    # print(new_OMF_pd_lip_nan)
    # name = ['CVAO'],#'Chichi'
    name = 'CVAO'
    dict_comp_lip = {
        name: {'lip_mod': list(OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_lipi_sub'])['OMF_mod_lip'].values),
               'lip_obs': list(
                   OMF_pd[OMF_pd['ID'] == name].dropna(subset=['OMF_obs_lipi_sub'])['OMF_obs_lipi_sub'].values)}}
...
def rename_func(data_pd, na, new_na):
    pd_new = data_pd
    list_col = list(data_pd['Measurements'].values)
...
def pd_combine_all(dicc_po_to, dicc_pr, dicc_li, plot_dir):
    ...
    omf_pd_po = create_dataframe(dicc_po_to, 'OMF_mod_poly', 'OMF_obs_poly_sub', mac_names[0])
    omf_pd_pr = create_dataframe(dicc_pr, 'OMF_mod_pro', 'OMF_obs_prot_sub', mac_names[1] )
    omf_pd_li = create_dataframe(dicc_li, 'OMF_mod_lip', 'OMF_obs_lipi_sub',  mac_names[2])
    omf_pd_to = create_dataframe(dicc_po_to, 'OMF_mod_tot', 'OMF_obs_tot_sub', mac_names[3])

    omf_pd_li_new = rename_func(omf_pd_li, 'CVAO','CVAO ')
    omf_pd_pr_new = rename_func(omf_pd_pr, 'CVAO', 'CVAO  ')
    omf_pd_to1 = rename_func(omf_pd_to, 'CVAO', 'CVAO   ')
    omf_pd_to2 = rename_func(omf_pd_to1, 'NAO', 'NAO ')
    omf_pd_to_new = rename_func(omf_pd_to2, 'WAP', 'WAP ')

    plot_functions.box_plot_vert(pd.concat([omf_pd_po, omf_pd_pr_new, omf_pd_li_new, omf_pd_to_new]),
                  mac_names, ['pol', 'pro', 'lip'],
                  'All_groups', [1e-5, 1e1])

    omf_pd_po.to_pickle('poly_omf.pkl')
    omf_pd_pr_new.to_pickle('prot_omf.pkl')
    omf_pd_li_new.to_pickle('lipi_omf.pkl')
    omf_pd_to_new.to_pickle('tot_omf.pkl')
